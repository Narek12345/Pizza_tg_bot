...
async def load_description(message: types.Message, state: FSMAdmin):
    """Принимаем описание."""
    if message.from_user.id == ID:
        async with state.proxy() as data:
...
async def cancel_handler(message: types.Message, state: FSMContext):
    """Выход из состояния машинных состояний."""
    if message.from_user.id == ID:
...
async def del_callback_run(callback_query: types.CallbackQuery):
    ...
    await callback_query.answer(text=f'{callback_query.data.replace("del ", "del")} удалена.', show_alert=True)
